...
**下一步**：① persona 仍依赖 keeper 给 NPC 填 `personality`——补"让 keeper 给玩出来的有戏份 NPC 固化一份性格"，否则权威块无性格可显示（石根/钱师傅那类）；② 接管在场/重要 NPC 名单（目前仍走线上 state）；③ 时机成熟后删除旧 reconciliation 阶段（clobber loop、`important_npc_tracker`、`continuity_resolver`）。观察/开关见 `doc/OPERATIONS.md`。
