...
- **P1 长尾召回 `retrieve()`（commit `df2641c` / `2d09448` / `8c11415`）**：设计文档的第三个纯函数落地为 `backend/fact_retrieval.py`——BM25 词法 + 实体链接 + 近窗，三车道 **RRF** 融合，不引任何新依赖（网关无 embedding 模型、机器上也无 embedding 运行时，见下）。
  - **实测才发现的坑（依次被真实数据推翻）**：拿 c701f6（93 facts/39 轮）+ 21 条手标 query 跑分，对比它要替掉的 bigram 集合重叠基线：① 车道等权 + 按 turn 破平局 → 头部全被近窗废话占住（MRR 0.26，**比基线还差**）；② `present` 行入 BM25 索引 → 只有"标签+地点"十来个 token，被长度归一化捧到长段观察之上；③ 实体车道 durable 严格优先 → 某 NPC 八条 `knows` 吃满配额、自己的戏份进不来；④ 全局 avgdl → `knows` 行系统性压过 observation，改**按谓词分组归一**；⑤ **RRF k=60 是错的**（TREC 上千文档的标度，在 ~100 条事实上 1/(60+1)≈1/(60+15)，于是"两车道各排中游"压过"词法第一"），k=2 才对，MRR 0.48→0.71，折半检验一致；⑥ **"改写型只能靠 embedding"也是错的**——bigram 窗口把单字语义切开（赏钱/谢仙师赏、护身/护盾、骨头/鸟骨），补单字 token 后 3 条彻底漏召全救回。
  - **跑分结果**（`scripts/recall_bench.py`，同池对比）：overall recall@3 0.90 / recall@8 **1.00** / MRR **0.825** / 漏召 **0**，基线 0.81 / 0.81 / 0.759 / 3。分层：**改写型 12 条 MRR 0.840 vs 基线 0.676**（这才是原始抱怨），逐字型 9 条 0.806 vs 基线 0.870（基线主场）。`--gate` currently PASS，但注入仍默认关（一个档 + 自标 21 条不足以改 narrator 行为，先用影子攒真实证据）。
