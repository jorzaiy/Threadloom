...
- 记忆 V2 长尾召回双轨（`retrieve()`，**影子默认开 / 注入默认关**）：
  ...
  - `THREADLOOM_RETRIEVE_V2`（默认 **0**）→ 把【往事回溯·检索】块注入 narrator（紧跟【人物档案·权威】）。**先看影子日志、跑召回基准，再考虑开**：`python3 scripts/recall_bench.py --data-root /root/Threadloom`（`runtime-data/` 被 gitignore，从 worktree 跑要指 `--data-root`；`--gate` 会在没超过词法基线时非零退出）。当前 21 条手标 query 上 PASS（MRR 0.825 vs 0.759、recall@8 1.00 vs 0.81、漏召 0 vs 3），但只覆盖一个档，故仍默认关。
