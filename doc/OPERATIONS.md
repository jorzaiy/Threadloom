...
- 记忆 V2 长尾召回双轨（`retrieve()`，**影子默认开 / 注入默认关**）：
  ...
  - `THREADLOOM_RETRIEVE_V2`（默认 **0**）→ 把【往事回溯·检索】块注入 narrator（紧跟【人物档案·权威】）。**先看影子日志、跑召回基准，再考虑开**：`python3 scripts/recall_bench.py --data-root /root/Threadloom`（`runtime-data/` 被 gitignore，从 worktree 跑要指 `--data-root`；`--gate` 会在没超过词法基线时非零退出）。当前 **57 条手标 query / 3 个档**上 PASS（MRR 0.838 vs 0.700、recall@8 0.96 vs 0.75、彻底漏召 1 vs 10），逐档单看也都赢（0.861/0.866/0.782 对 0.759/0.725/0.606）。仍默认关的原因**不再是"只覆盖一个档"，而是至今没有任何真实游玩的影子日志**——线上跑的 memory-v2 还没合入这条分支，`retrieve_shadow.jsonl` 一份都没落盘。
